...
`scripts/build-emacs-browser-asyncify-spike.sh` accepts
...
`scripts/wasmacs-asyncify-host-library.js`. It is intentionally a no-op async
...
The copied-source patch in `scripts/patch-emacs-host-entrypoint-spike.sh`
...
repo-local JS library. `scripts/validate-minibuffer-asyncify-entrypoint-plan.sh`
checks the plan, local Emscripten settings, artifact shape, wait import,
exports, and batch boot log.

`scripts/probe-browser-asyncify-interactive-start.mjs` covers the first
...
`scripts/probe-browser-asyncify-minibuffer-waitpoint.mjs` covers the next
...
`read-from-minibuffer`, but `scripts/probe-browser-asyncify-minibuffer-suspend-state.mjs`
...
`scripts/probe-browser-asyncify-minibuffer-input-injection.mjs` covers the
...
`scripts/probe-browser-asyncify-minibuffer-cancel.mjs` covers the cancel side.
...
`scripts/summarize-asyncify-advise.mjs` is a diagnostic companion for that
