...
out_dir="${repo_root}/build/artifacts/emacs-browser-pdump-interactive"
asyncify_lib="${repo_root}/tools/scripts/wasmacs-asyncify-host-library.js"
...
if [ ! -f "${build_dir}/src/Makefile" ]; then
  echo "error: pdump build tree missing; run tools/scripts/probe-emacs-pdump-temacs-build.sh first" >&2
  exit 1
fi
