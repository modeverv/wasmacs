...
doc="${repo_root}/doc/minibuffer-suspended-read-plan.md"
probe="${repo_root}/tools/scripts/probe-browser-minibuffer-state-export.mjs"
active_probe="${repo_root}/tools/scripts/probe-browser-minibuffer-active-read-boundary.mjs"
log="${repo_root}/logs/wasm-browser-minibuffer-state-export.txt"
active_log="${repo_root}/logs/wasm-browser-minibuffer-active-read-boundary.txt"

if [[ ! -f "${log}" ]]; then
  log="${repo_root}/archive/old-logs/wasm-browser-minibuffer-state-export.txt"
fi

if [[ ! -f "${active_log}" ]]; then
  active_log="${repo_root}/archive/old-logs/wasm-browser-minibuffer-active-read-boundary.txt"
fi
...
rg 'wasmacs_command_begin_minibuffer_probe' "${doc}" >/dev/null
rg 'unavailable:noninteractive-batch' "${doc}" >/dev/null
rg 'probe-browser-minibuffer-state-export\.mjs' "${doc}" >/dev/null
rg 'probe-browser-minibuffer-active-read-boundary\.mjs' "${doc}" >/dev/null
rg 'wasm-browser-minibuffer-state-export\.txt' "${doc}" >/dev/null
rg 'wasm-browser-minibuffer-active-read-boundary\.txt' "${doc}" >/dev/null

rg 'wasmacs_minibuffer_state' "${repo_root}/tools/scripts/patch-emacs-host-entrypoint-spike.sh" >/dev/null
rg 'wasmacs_entrypoint_state' "${repo_root}/tools/scripts/patch-emacs-host-entrypoint-spike.sh" >/dev/null
rg 'wasmacs_command_begin_minibuffer_probe' "${repo_root}/tools/scripts/patch-emacs-host-entrypoint-spike.sh" >/dev/null
rg 'unavailable:busy' "${repo_root}/tools/scripts/patch-emacs-host-entrypoint-spike.sh" >/dev/null
rg '_wasmacs_minibuffer_state' "${repo_root}/tools/scripts/build-emacs-browser-persistent-spike.sh" >/dev/null
rg '_wasmacs_entrypoint_state' "${repo_root}/tools/scripts/build-emacs-browser-persistent-spike.sh" >/dev/null
rg '_wasmacs_command_begin_minibuffer_probe' "${repo_root}/tools/scripts/build-emacs-browser-persistent-spike.sh" >/dev/null
...
rg 'BEGIN_STATUS:3' "${active_log}" >/dev/null
rg 'BEGIN_READBACK:unavailable:noninteractive-batch' "${active_log}" >/dev/null
rg 'AFTER_COMMAND_STATE:idle' "${active_log}" >/dev/null
rg 'AFTER_MINIBUFFER_STATE:active:false' "${active_log}" >/dev/null
