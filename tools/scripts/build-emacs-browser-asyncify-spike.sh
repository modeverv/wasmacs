...
vscode_build_root="${WASMACS_VSCODE_BUILD_ROOT:-${repo_root}/build2}"
work_root="${vscode_build_root}/emacs-browser-asyncify-spike"
build_dir="${work_root}/build-gnu-host-internal-termcap"
source_copy="${work_root}/src"
out_dir="${vscode_build_root}/artifacts/emacs-browser-asyncify-spike"
...
native_baseline="${WASMACS_NATIVE_BASELINE:-${vscode_build_root}/native-emacs-30.2/src}"
...
if [ ! -x "${native_baseline}/lib-src/make-docfile" ] || [ ! -x "${native_baseline}/lib-src/make-fingerprint" ]; then
  WASMACS_NATIVE_WORK_ROOT="${vscode_build_root}/native-emacs-30.2" \
  WASMACS_NATIVE_LOG_FILE="${repo_root}/logs/vscode-native-baseline.txt" \
