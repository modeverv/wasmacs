...
out_dir="${repo_root}/build/artifacts/emacs-browser-runtime"
...
if [ ! -f "${build_dir}/src/Makefile" ]; then
  echo "error: pdump configure probe build not found; run tools/scripts/probe-emacs-pdump-temacs-build.sh first" >&2
  exit 1
fi
