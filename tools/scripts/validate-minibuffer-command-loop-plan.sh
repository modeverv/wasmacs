...
doc="${repo_root}/doc/minibuffer-command-loop-plan.md"
probe="${repo_root}/tools/scripts/probe-browser-minibuffer-state.mjs"
log="${repo_root}/logs/wasm-browser-minibuffer-state.txt"

if [[ ! -f "${log}" ]]; then
  log="${repo_root}/archive/old-logs/wasm-browser-minibuffer-state.txt"
fi
