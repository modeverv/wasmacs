...
描画方式は最初は DOM/CSS grid よりも Canvas/WebGL text atlas を優先したい。Emacs の redisplay は細かい invalidation が多く、DOM ノード大量更新は早めに苦しくなる可能性が高い。ただし MVP では DOM textarea と overlay で始める余地はある。
...
- pdumper/dumped image の初期対応

これらは夢の否定ではなく、最初の動く核を守るための境界である。
