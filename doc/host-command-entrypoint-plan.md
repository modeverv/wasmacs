...
`tools/scripts/probe-browser-persistent-callmain.mjs` shows:
...
`tools/scripts/patch-emacs-host-entrypoint-spike.sh` patches only:
...
`tools/scripts/probe-browser-host-entrypoint.mjs` verifies the shape:
...
`tools/scripts/probe-browser-host-file-command.mjs` proves the file side of that
path:
...
`tools/scripts/probe-browser-host-readback.mjs` proves the first dedicated readback
shape:
...
`tools/scripts/probe-browser-file-buffer-gc-roots.mjs` narrows the persistent-buffer
...
`tools/scripts/probe-browser-find-file-phases.mjs` shows `find-file-noselect`,
...
`tools/scripts/probe-browser-undo-tail-phases.mjs` adds the next undo boundary:
