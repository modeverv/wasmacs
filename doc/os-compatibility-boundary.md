...
source scripts such as `tools/scripts/patch-emacs-host-entrypoint-spike.sh` until an
...
| Entry point root refresh | Some exports refresh stack bounds through local macros, but the named product facade is not yet a uniform begin/end pair. | `tools/scripts/patch-emacs-host-entrypoint-spike.sh` defines `WASMACS_ENTER_HOST_ENTRYPOINT` and `WASMACS_LEAVE_HOST_ENTRYPOINT`. | Add diagnostic begin/end facade names and stack/root probes before widening exported calls. |
...
| Preloaded state / pdump | Browser worker cold loadup and pdump evidence are diagnostic lanes, not current product startup. | `doc/small-os-for-emacs.md` demotes pdmp/pbootstrap; terminal path is current product route. | Do not resume pdump as product work until terminal route fails with source-backed evidence or user asks. |
...
`tools/scripts/patch-emacs-host-entrypoint-spike.sh`. These entrypoints are exported
...
```sh
node tools/scripts/probe-browser-os-diagnostic-facade.mjs
```

## Resume Memory/Root Comparison Probe

2026-06-03 added `tools/scripts/probe-browser-os-resume-memory-root.mjs` as a
...
2026-06-03 added `tools/scripts/probe-browser-blocking-input-scheduler.mjs` as a
...
`tools/scripts/probe-asyncify-import-contract.mjs` shows that `ASYNCIFY_IMPORTS`
...
`tools/scripts/probe-browser-blocking-input-scheduler.mjs` records both modes into
...
2026-06-03 added `tools/scripts/probe-browser-asyncify-outer-resume.mjs` and
`tools/scripts/probe-browser-blocking-input-scheduler.mjs` (updated) as a
...
Probe: `tools/scripts/probe-browser-asyncify-outer-resume.mjs`  
...
it. The accessor is defined in `tools/scripts/wasmacs-asyncify-host-library.js` and
...
Probe: `tools/scripts/probe-browser-blocking-input-handleasync-loop.mjs`  
...
Probe: `tools/scripts/probe-browser-worker-handleasync-input-smoke.mjs`  
...
Probe: `tools/scripts/probe-browser-keyboard-event-semantics.mjs`  
...
`tools/scripts/probe-browser-xterm-terminal-smoke.mjs` verifies:
...
`tools/scripts/probe-browser-xterm-redraw-fidelity.mjs` verifies ANSI sequence quality and full edit sequence:
...
`tools/scripts/probe-browser-xterm-product-editing-smoke.mjs` verifies the architectural invariant:
...
`tools/scripts/probe-browser-xterm-manual-app-smoke.mjs`:
