...
**`tools/scripts/wasmacs-atomics-host-library.js`** — ソース（次ビルドで反映）
...
**`build/artifacts/emacs-browser-atomics/temacs`** — ホットパッチ済み（即時有効）
...
- `doc/small-os-for-emacs.md` の Terminal/Tty Service 設計と整合する
...
- `tools/scripts/wasmacs-atomics-host-library.js` — FIONREAD ioctl 追加、queue 注入に変更
- `tools/scripts/patch-emacs-host-entrypoint-spike.sh` — `os-compat` モード追加
- `tools/scripts/build-emacs-browser-atomics.sh` — `os-compat` モードを使用
