...
`build/artifacts/emacs-core-spike.js` is a Node smoke artifact. It links with
...
- `build/artifacts/emacs-core-spike.js` contains `NODERAWFS`.
...
`tools/scripts/build-emacs-browser-profile-spike.sh` builds the first browser-shaped
artifact directory:
...
See `doc/persistent-command-loop-feasibility.md` for the current persistent
...
`build/artifacts/emacs-browser-persistent-spike/`, built by
`tools/scripts/build-emacs-browser-persistent-spike.sh`. It uses
`-sEXIT_RUNTIME=0`, exports `callMain`, `wasmacs_eval_string`,
`wasmacs_last_result`, and Emscripten FS helpers including `FS_readFile`.
Validation is in `tools/scripts/validate-browser-persistent-spike.sh` and
...
`tools/scripts/summarize-browser-editing-session.mjs` rolls the individual browser
...
`doc/clipboard-kill-ring-boundary.md`; browser evidence is in
...
`doc/minibuffer-command-boundary.md`; deterministic probe evidence is in
...
`doc/minibuffer-command-loop-plan.md` defines the next real-minibuffer
boundary as Emacs-owned `host.gui.minibuffer-state` /
`host.gui.minibuffer-input` messages. `tools/scripts/probe-browser-minibuffer-state.mjs`
...
`tools/scripts/validate-browser-editing-smoke-evidence.sh`. It also starts
`tools/scripts/serve-app.mjs` automatically when the target app server is not already
running.

The next shared dependency for undo, kill-ring, and minibuffer fidelity is a
stable persistent Emacs buffer path.
`doc/persistent-emacs-buffer-requirement.md` records why the current
...
`tools/scripts/probe-browser-persistent-buffer-matrix.mjs` narrows this further:
...
UI. `tools/scripts/probe-browser-worker-repeated-undo.mjs` and
...
`undo-redo 1`; `tools/scripts/probe-browser-worker-redo.mjs` and
`logs/browser-redo-ui-smoke.txt` prove insert/undo/redo through the worker and
browser UI. `tools/scripts/probe-browser-worker-redo-interleaving.mjs` extends that
...
externally changed. `tools/scripts/probe-browser-worker-point-undo-redo.mjs` adds a
...
file-visiting buffer. `tools/scripts/probe-browser-worker-file-switch-undo.mjs`
...
```sh
tools/scripts/validate-browser-mvp-readiness.sh
tools/scripts/validate-browser-profile-spike.sh
tools/scripts/validate-browser-persistent-spike.sh
tools/scripts/validate-browser-worker-app.sh
npm test
```
