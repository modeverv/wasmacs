...
The scaffold already passes VS Code webview URIs for the existing browser
runtime assets:
...
extension uses a non-Atomics route when `build/artifacts/emacs-browser-asyncify-spike`
is available. If that artifact is missing, it selects an `extension-host-bridge`
placeholder and reports the build/package step as the blocker.
